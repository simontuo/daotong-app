...
<script>
    export default {
        ...
        methods: {
            search () {
                axios.get('/api/admin/answers/search', {'params': {'query': this.query, 'quickQuery': this.quickQuery, 'pageSize': this.pageSize}}).then(response => {
                    this.data = response.data.answers.data;
                    this.total = parseInt(response.data.answers.total);
                    this.loading = false;
                });
            },
            changePage (page) {
                axios.get('/api/admin/answers/search', {params: {'query': this.query, 'quickQuery': this.quickQuery, 'page': page, 'pageSize': this.pageSize}}).then(response => {
                    this.data = response.data.answers.data;
                    this.total = parseInt(response.data.answers.total);
                    this.loading = false;
                });
            ...
            pageSizeChange (pageSize) {
                ...
                axios.get('/api/admin/answers/search', {params: {'query': this.query, 'quickQuery': this.quickQuery, 'page': 1, 'pageSize': this.pageSize}}).then(response => {
                    this.data = response.data.answers.data;
                    this.total = parseInt(response.data.answers.total);
                    this.loading = false;
                });
