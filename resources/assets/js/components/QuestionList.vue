<template>
    <div class="">
        ...
        <div class="row">
            ...
            <div class="col-md-4 visible-xs" v-for="item in questions">
            ...
            <div class="col-md-9 mdui-m-b-2 hidden-xs">
                <ul class="media-list article-list mdui-m-y-1">
                    <div class="article-list-hover">
                        ...
                        <li class="media" style="margin-top: 0px;" v-for="item in questions">
                            ...
                            <div class="media-body">
                                <p class="media-heading mdui-typo mdui-typo-subheading" style="line-height:1">
                                    <a :href="'/questions/' + item.id" class="mdui-text-color-grey-800">{{ item.title }}</a>
...
<script>
    export default {
        ...
        mounted() {
            axios.get('api/questions/search', {'params': {'query': this.query, 'quickQuery':            this.quickQuery}}).then(response => {
                this.questions = response.data.questions.data;
                if (!response.data.questions.next_page_url) {
                    this.noMoreData = true;
                }
                this.nextPageUrl = response.data.questions.next_page_url;
                this.loading = false;
        ...
        methods: {
            toLoading () {
                ...
                axios.get(this.nextPageUrl).then(response => {
                    this.questions = this.questions.concat(response.data.questions.data);
                    if (!response.data.questions.next_page_url) {
                        this.noMoreData = true;
                    }
                    this.nextPageUrl = response.data.questions.next_page_url;
